...
// Function clear globar variables
const emptyGlobaVars = () => {
  // global variable for inputs values
  countryCode = "US"; // by default
  searchInputs = "";
  searchedTopic = [];
...
const openweatherAPI = () => {
  ...
  fetch(queryURL)
    ...
    .then(function(res) {
      ...
      var displayTempMax = document.createElement('p');
      var dispalyTempMin = document.createElement('p');
      var displayHumidity = document.createElement('p');
      var dispalyWind = document.createElement('p');

  
      title.textContent = `Weather report in ${searchInputs}`;
      displayTemp.textContent = `Today's temperate in ${searchInputs} is ${temp} F`;
      displayTempMax.textContent = `The maximum temperature is ${tempMax} F`;
      dispalyTempMin.textContent = `The miniumum temperature is ${tempMin} F`;
      displayHumidity.textContent = `The humidity is ${humidity} %`;
      dispalyWind.textContent = `The wind speed is ${wind} mph`;

      containerContent.append(title);
      containerContent.append(displayTemp);
      containerContent.append(displayTempMax);
      containerContent.append(dispalyTempMin);
      containerContent.append(displayHumidity);
      containerContent.append(dispalyWind);
